...
router.get("/:chatID", tokenExtractor, userExtractor, async (request, response) => {
    ...
    if (memberError) return response.status(400).json(memberError);

    if (!members || members.length === 0) return response.status(400).json({error: "You are not authorized to perform this action."});
    ...
    const formattedMessages = MessageSchema.array().parse(messages);
